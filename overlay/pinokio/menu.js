module.exports = async (kernel, info) =>
{
	...
	if (!info.running('run.js') && !info.running('install.js') && !info.running('update.js') && !info.running('reset.js'))
	{
		[
			// Quality profiles
			[ 'fa-solid fa-bolt', 'Fast (Deck)', 'Fast' ],
			[ 'fa-solid fa-power-off', 'Balanced (Deck)', 'Balanced' ],
			[ 'fa-solid fa-gem', 'Quality (Deck)', 'Quality' ],
			// Extras
			[ 'fa-solid fa-robot', 'Balanced + Jobs', 'Default+Jobs' ],
			[ 'fa-solid fa-gauge', 'Benchmark', 'Benchmark' ],
			[ 'fa-solid fa-camera', 'Webcam (Fast)', 'Webcam' ]
		]
